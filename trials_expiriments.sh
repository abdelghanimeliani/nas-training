...
for trials in "${TRIALS[@]}"; do
    for method in "${SEARCH_METHODS[@]}"; do
        EXP_NAME="exp_${method}_${trials}_trials_"
        echo "Running: $EXP_NAME on port $PORT with $trials trials and method $method"
        # Run the Python runner and get experiment IDs
        EXP_IDS=$(python3 runner.py  --experiment-name "exp_{$method}_optimizer_with_{$trials}_steps_with_"  --max-trials $trials  --optimizer $method --max-duration 360000s | sed -r 's/\x1B\[[0-9;]*[A-Za-z]//g' | grep -oP "(?<=Experiment ID: )\S+") 
