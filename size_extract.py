...
def extract_size_results():
    # Updated paths for the new size-based experiments
    base_dir = "./size_results/new_metric_data"
    output_csv = "size_results.csv"
    ...
    # Define our target structural keys (Swapped Horizons for Data Sizes)
    SEARCH_METHODS = ["random", "GridSearch", "tpe", "anneal", "evolution"]
    DATA_SIZES = [10, 20, 50, 100, 200, 300, 400, 500]
    
    # Sort datasets by length descending so "local_plus_ali" matches before "ali_only"
    # Added your new bash script datasets here as well just in case!
    DATASETS_ORDERED = [
    ...
    # Key: (dataset_name, method, data_size) -> [min_mape, min_mae, min_mse]
    database = {}
    ...
    for f_name in all_files:
        ...
        # Detect the data size step number (extracting the number following the 'sz' prefix)
        prefix = f"exp_{matched_method}_sz"
        if prefix not in f_name:
            continue
            
        remaining_str = f_name[len(prefix):] # e.g., "10_local_only.csv"
        size_str = remaining_str.split("_")[0]
        try:
            matched_size = int(size_str)
        except ValueError:
            continue
        ...
        try:
            ...
            if file_min_mape != float('inf'):
                combo_key = (matched_dataset, matched_method, matched_size)
    ...
    DATASETS_ORIGINAL_FORMAT = [
        "local_only", "tt_only", "local_plus_tt", "mat_only", "ali_only", 
        "local_plus_mat", "local_plus_ali"
    ]

    with open(output_csv, "w", newline="", encoding="utf-8") as out_f:
        writer = csv.writer(out_f)
        # Updated header to use "Data_Size" instead of "Horizon"
        writer.writerow(["Dataset", "Method", "Data_Size", "Min_MAPE", "Min_MAE", "Min_MSE"])

        for dataset in DATASETS_ORIGINAL_FORMAT:
            for method in SEARCH_METHODS:
                for size in DATA_SIZES:
                    combo_key = (dataset, method, size)
